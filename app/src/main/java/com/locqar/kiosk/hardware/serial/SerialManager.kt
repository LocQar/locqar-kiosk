package com.locqar.kiosk.hardware.serial
...
/**
 * Manages USB-to-RS485 serial connection for Winnsen lock controller boards.
 *
 * Serial config: 9600 baud, 8 data bits, 1 stop bit, no parity.
 * Adapters: CH340, CH341, PL2303, FTDI, CP210x.
 */
class SerialManager(private val context: Context) {

    companion object {
        private const val TAG = "SerialManager"
        private const val ACTION_USB_PERMISSION = "com.locqar.kiosk.USB_PERMISSION"
    ...
    /**
     * Scan for USB serial devices and connect to the first one found.
     */
    fun connect() {
    ...
    private fun openDevice(device: UsbDevice) {
        try {
            ...
            val driver = drivers.find { it.device == device }
                ?: throw IllegalStateException("Driver not found for device")
            ...
            _connectionState.value = ConnectionState.ERROR
            _errorMessage.value = "Failed to open: ${e.message}"
     ...
     *
     * RS485 responses may arrive in multiple chunks, so we accumulate
     * bytes until we have the expected response length or timeout.
     *
     * @param txData Command bytes to send
     * @param expectedResponseLen Expected number of bytes in the response
    ...
    fun sendAndReceive(txData: ByteArray, expectedResponseLen: Int): ByteArray? {
        ...
        return try {
            ...
            // Read response — accumulate chunks until we have enough bytes
